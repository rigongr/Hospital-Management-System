<template>
  <div>
    <NewHeader />
    <PatientSideBar />
    <h1 style="margin-left: 350px">Doktoret</h1>
    <b-row style="margin-left: 350px">
      <b-col>
        <b-card v-for="item in data" :key="item.id">
          <!-- <h1 :to="{ name: '/patient/department', params: { id: item.id } }">{{item.name}}</h1> -->
          <h1 @click="patientDoctors(item.id)">{{item.name}}</h1>
...
import PatientSideBar from "../layouts/PatientSideBar.vue";
...
export default {
  ...
  data() {
    return {
      ...
      id: 'asd123',
      name: 'Doktori 1',
      description: 'Ne Departamentin e Kirurgjise'
    },
    {
      id: 'asd321',
      name: 'Doktori 2',
      description: 'Ne Departamentin e Stomatologjise'
    }]
      // total: {},
      // unassigned: {},
      // overdue: {},
  ...
  methods: {
    ...
    patientDepartment(id) {
      this.$router.push(`/patient/doctors/${id}`)
    }
  },
