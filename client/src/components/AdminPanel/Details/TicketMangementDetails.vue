<template>
  <div>
    <NewHeader />
    <AdminSideBar />
    <div v-for="ticket in allTickets" :key="ticket._id">
      <b-card-group deck style="margin: 20px 50px 0 400px" v-if="ticket._id === ticketId">
        <b-card header="Ticket Details">
          <b-row>
            <b-col>
            ...
            <b-col>
              ...
              <input type="text" id="example1" class="form-control" :value="ticket.assigned" />
              <b-form-checkbox
                id="checkbox-1"
                v-model="overdue"
                name="checkbox-1"
              ...
              <b-form-checkbox
                ...
                unchecked-value="not_accepted"
                style="margin-top: 40px"
              >Close Ticket</b-form-checkbox>
            </b-col>
          </b-row>
          <br />
          <b-button size="sm" variant="primary" @click="handleUpdateTicket">Save</b-button>
...
export default {
  ...
  methods: {
    ...
    handleUpdateTicket(evt) {
      ...
      this.updateTicket(bodyData);
      router.go(-1);
    }
  },
  computed: mapGetters(["allTickets"]),
  mounted() {
    this.ticketId = this.$route.params.prodId;
    this.value = axios
      .get("http://localhost:5000/api/tickets/" + this.ticketId)
      .then(response => {
