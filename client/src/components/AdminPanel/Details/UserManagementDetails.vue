<template>
  <div>
    ...
    <div v-for="user in allUsers" :key="user._id">
      <b-card-group deck style="margin: 20px 50px 0 400px" v-if="user._id === userId">
        <b-card header="User Details">
          <b-row>
            ...
            <b-col>
              ...
              <input type="text" id="example1" class="form-control" :value="user.assigned" />
              <b-form-checkbox
                id="checkbox-1"
                v-model="overdue"
                name="checkbox-1"
              ...
              <b-form-checkbox
                ...
                unchecked-value="not_accepted"
                style="margin-top: 40px"
              >Close User</b-form-checkbox>
            </b-col>
          </b-row>
          <br />
          <b-button size="sm" variant="primary" @click="handleUpdateUser">Save</b-button>
...
export default {
  ...
  methods: {
    ...
    handleUpdateUser(evt) {
      ...
      this.updateUser(bodyData);
      router.go(-1);
    }
  },
  computed: mapGetters(["allUsers"]),
  mounted() {
    this.userId = this.$route.params.prodId;
    this.value = axios
