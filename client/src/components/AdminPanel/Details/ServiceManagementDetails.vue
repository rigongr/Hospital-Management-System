<template>
  <div>
    <NewHeader />
    <AdminSideBar />
    <div v-for="service in allServices" :key="service._id">
      <b-card-group deck style="margin: 20px 50px 0 400px" v-if="service._id === serviceId">
        <b-card header="Service Details">
          <b-row>
            <b-col>
            ...
            <b-col>
              ...
              <input type="text" id="example1" class="form-control" :value="service.assigned" />
              <b-form-checkbox
                id="checkbox-1"
                v-model="overdue"
                name="checkbox-1"
              ...
              <b-form-checkbox
                ...
                unchecked-value="not_accepted"
                style="margin-top: 40px"
              >Close Service</b-form-checkbox>
            </b-col>
          </b-row>
          <br />
          <b-button size="sm" variant="primary" @click="handleUpdateService">Save</b-button>
...
export default {
  ...
  methods: {
    ...
    handleUpdateService(evt) {
      ...
      this.updateService(bodyData);
      router.go(-1);
    }
  },
  computed: mapGetters(["allServices"]),
  mounted() {
    this.serviceId = this.$route.params.prodId;
    this.value = axios
      .get("http://localhost:5000/api/services/" + this.serviceId)
      .then(response => {
